...
#training a Naive Bayes classifier using the curated training/test data and the computed feature vector
...
#nbRandom=RandomizedSearchCV(estimator=ber, param_distributions=parameters, n_iter=2, cv=2,verbose=2)
#cv.fit(X_train, y_train)
#print(nbRandom.get_params().keys())
#nbRandom.fit(X_train, y_train)
#
#
#def display(results):
#    print("\n")
...
y_score = ber.predict_proba(X_test)
...
plt.plot(recall, precision, marker='.', label='ROC Naive Bayes')
# axis labels
plt.xlabel('Recall')
...
#feature_importances = zip(finalColumns, ber.coef_.flatten())
#
...
#listFI=(np.std(X_featureVector, 0)*ber.coef_.flatten())
#
